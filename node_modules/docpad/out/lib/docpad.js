...
(function() {
  ...
  DocPad = (function(superClass) {
    ...
    DocPad.prototype.klass = DocPad;

    DocPad.DocPad = DocPad;

    DocPad.create = function(instanceConfig, next) {
      return new DocPad(instanceConfig, next);
    };

    DocPad.createInstance = function(instanceConfig, next) {
      return new DocPad(instanceConfig, next);
    };
    ...
    function DocPad(instanceConfig, next) {
      var action, configEventContext, docpad, i, len, logger, loggerConsole, loggers, methodName, ref1, ref2;
      ...
      this.database = new FilesCollection(null, {
        ...
        var j, len1, outPath, ref3, updatedModels, url;
        ...
        var j, k, len1, len2, ref3, ref4, url;
        if (model.get('write') === false) {
          return;
        }
        ref3 = model.previous('urls') || [];
        for (j = 0, len1 = ref3.length; j < len1; j++) {
          url = ref3[j];
          delete docpad.filesByUrl[url];
        }
    ...
    DocPad.prototype.generate = function(opts, next) {
      ...
      tasks = new TaskGroup("generate tasks").on('item.run', function(item) {
        ...
        return (ref3 = opts.progress) != null ? ref3.step("generate: " + (item.getConfig().name)).total(totals.total).setTick(totals.completed) : void 0;
      }).done(function(err) {
